    ...
    public:
        ...
        const Eigen::SparseMatrix<T> ComputeStiffnessMatrix(const Matrix3X<T>& vertices) const {
            ...
            for (int i = 0;i < tet_num;i++) {
                ...
                // compute tet element i's contribution to the stiffness matrix K   
                // Ki = d^2 E / d x^2
                Eigen::Matrix<T, 9, 12> dPdx_i = material.StressDifferential(F) * dFdx[i];
                Eigen::Matrix<T, 12, 12> Ki = tet_volume[i] * dPdx_i.transpose() * dFdx[i];

                const int dim = 3;
                for (int x = 0; x < 4; x++) 
                    for (int y = 0; y < 4; y++) 
                        for (int x_dim = 0; x_dim < dim; x_dim++)
                            for (int y_dim = 0; y_dim < dim; y_dim++) {
                                T val = Ki(x * dim + x_dim, y * dim + y_dim);
                                if (abs(val) >= 1e-9) 
                                    triplet_list.push_back(Eigen::Triplet<T>(elements[x] * dim + x_dim, elements[y] * dim + y_dim, val));
                            }
            }
