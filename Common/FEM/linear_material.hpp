...
namespace materials {
    ...
    public:
        ...
        const typename Material<dim, T>::MatrixDim2T StressDifferential(
            ...
            const typename Material<dim, T>::MatrixDim2T dFdF = Material<dim, T>::MatrixDim2T::Identity();
            typename Material<dim, T>::MatrixDim2T dFTdF = Material<dim, T>::MatrixDim2T::Zero();
            typename Material<dim, T>::MatrixDim2T dTrFdF = Material<dim, T>::MatrixDim2T::Zero();
            
            for (int i = 0; i < dim; i++) {
                for (int j = 0; j < dim; j++) {
                    dFTdF(i * dim + j, j * dim + i) = 1;
                    dTrFdF(i * dim + i, j * dim + j) = 1;
                }
            }

            return mu * (dFdF + dFTdF) + lambda * dTrFdF;
        }
