        ...
        */
        ...
        void CreateContour(mesh::TriMesh<T>& tri_mesh,
            ...
            for (int i = 0; i < intersection_edges.size(); i++) {
                ...
                std::vector<IntersectionEdge<T>> intersections_one_plane = intersection_edges[i];

                std::vector<Vector3<T>> points;
                std::vector<bool> options;
                std::unordered_map<int, int[2]> edges;

                for (int j = 0; j < intersections_one_plane.size(); j++) {
                    IntersectionEdge<T> edge = intersections_one_plane[j];

                    int idx0 = -1; int idx1 = -1;
                    for (int k = 0; k < points.size(); k++) {
                        if (!options[k]) continue;
                        if ((points[k] - edge.p0()).norm() < epsilon) {
                            idx0 = k; break;
                        }
                    }
                    for (int k = 0; k < points.size(); k++) {
                        if (!options[k]) continue;
                        if ((points[k] - edge.p1()).norm() < epsilon) {
                            idx1 = k; break;
                        }
                    }
                    if (idx0 == -1) {
                    ...
                    if (edges.find(idx0) == edges.end()) edges[idx0][0] = idx1;
                    else {
                        edges[idx0][1] = idx1;
                        options[idx0] = false;
                    }
                    if (edges.find(idx1) == edges.end()) edges[idx1][0] = idx0;
                    else {
                        edges[idx1][1] = idx0;
                        options[idx1] = false;
                    }
                ...
                for (int l = 0; l < points.size(); l++) {
                    ...
                    if (std::find(added.begin(), added.end(), l) != added.end()) continue;
                    if (added.size() == points.size()) break;

                    std::vector<Vector3<T>> contour{ points[l], points[edges[l][0]]};
                    added.push_back(l); 

                    int prev = l; int current = edges[prev][0];
                    int next = edges[current][0] == prev ? edges[current][1] : edges[current][0];

                    while (std::find(added.begin(), added.end(), next) == added.end()) {
                        contour.push_back(points[next]);
                        added.push_back(next);
                        prev = current; current = next; 
                        next = edges[current][0] == prev ? edges[current][1] : edges[current][0];
                    }
        ...
        void Infill(std::vector<std::vector<std::vector<Vector3<T>>>>& contours,
            ...
            for (int i = 0; i < contours.size(); i++) {
                std::vector<std::pair<Vector3<T>, Vector3<T>>> infill_one_layer;

                std::vector<std::vector<Vector3<T>>>& contours_one_layer = contours[i];
                T z = contours_one_layer[0][0][2];

                for (T x = _infill_x_lower_bound; x <= _infill_x_upper_bound; x += _infill_dx) {
                    std::vector<Vector3<T>> intersections;

                    for (int j = 0; j < contours_one_layer.size(); j++) {
                        std::vector<Vector3<T>>& contour = contours_one_layer[j];

                        for (int k = 0; k < contour.size(); k++) {
                            int next_k = (k + 1) % contour.size();
                            if ((contour[k][0] - x) * (contour[next_k][0] - x) <= 0) {
                                Vector3<T> dir = contour[next_k] - contour[k];
                                const T t = (x - contour[k][0]) / dir[0];
                                intersections.push_back(contour[k] + dir * t);
                            }
                        }
                    }

                    std::sort(intersections.begin(), intersections.end(), 
                        [](const Vector3<T> v1, const Vector3<T> v2) {return v1[1] > v2[1]; });

                    for (int l = 0; l < intersections.size(); l += 2) {
                        infill_one_layer.push_back(std::make_pair(intersections[l], intersections[l + 1]));
                    }
                }

                for (T y = _infill_y_lower_bound; y <= _infill_y_upper_bound; y += _infill_dx) {
                    std::vector<Vector3<T>> intersections;
                    ...
                    for (int j = 0; j < contours_one_layer.size(); j++) {
                        ...
                        for (int k = 0; k < contour.size(); k++) {
                            int next_k = (k + 1) % contour.size();
                            if ((contour[k][1] - y) * (contour[next_k][1] - y) <= 0) {
                                Vector3<T> dir = contour[next_k] - contour[k];
                                const T t = (y - contour[k][1]) / dir[1];
                                intersections.push_back(contour[k] + dir * t);
                            }
                        }
                    }
                    ...
                    std::sort(intersections.begin(), intersections.end(),
                        [](const Vector3<T> v1, const Vector3<T> v2) {return v1[0] > v2[0]; });

                    for (int l = 0; l < intersections.size(); l += 2) {
                        infill_one_layer.push_back(std::make_pair(intersections[l], intersections[l + 1]));
                    }
                }
                infill_edges.push_back(infill_one_layer);
            }
        }

        void CustomInfill(std::vector<std::vector<std::vector<Vector3<T>>>>& contours,
            std::vector<std::vector<std::pair<Vector3<T>, Vector3<T>>>>& infill_edges) {

            Eigen::Quaternion<double> q = Eigen::AngleAxisd(M_PI / 4.0, Eigen::Vector3d::UnitZ()) *
                Eigen::AngleAxisd(0, Eigen::Vector3d::UnitY()) *
                Eigen::AngleAxisd(0, Eigen::Vector3d::UnitX());
            Eigen::Matrix3d rotation = q.matrix();
            Eigen::Matrix3d rotation_reverse = rotation.inverse();

            Vector3<T> corner1(_infill_x_lower_bound, _infill_y_lower_bound, 0);
            Vector3<T> corner2(_infill_x_upper_bound, _infill_y_upper_bound, 0);
            Vector3<T> corner3(_infill_x_upper_bound, _infill_y_lower_bound, 0);
            Vector3<T> corner4(_infill_x_lower_bound, _infill_y_upper_bound, 0);

            std::vector<T> x_vals; std::vector<T> y_vals;
            
            for (Vector3<T> corner : { corner1, corner2, corner3, corner4 }) {
                corner = rotation * corner;
                x_vals.push_back(corner[0]);
                y_vals.push_back(corner[1]);
            }

            auto x_minmax = std::minmax_element(x_vals.begin(), x_vals.end());
            auto y_minmax = std::minmax_element(y_vals.begin(), y_vals.end());
            ...
            for (int i = 0; i < contours.size(); i++) {
                ...
                if (i % 2 == 0) {
                    ...
                    for (T x = *x_minmax.first; x <= *x_minmax.second; x += _infill_dx) {
                        std::vector<Vector3<T>> intersections;
                        ...
                        for (int j = 0; j < contours_one_layer.size(); j++) {
                            ...
                            for (int k = 0; k < contour.size(); k++) {
                                ...
                                Vector3<T> point1 = rotation * contour[k];
                                Vector3<T> point2 = rotation * contour[next_k];

                                if ((point1[0] - x) * (point2[0] - x) <= 0) {
                                    Vector3<T> dir = point2 - point1;
                                    const T t = (x - point1[0]) / dir[0];
                                    intersections.push_back(point1 + dir * t);
                                }
                            }
                        }
                        ...
                        for (int l = 0; l < intersections.size(); l += 2) {
                            infill_one_layer.push_back(std::make_pair(rotation_reverse * intersections[l], 
                                                                      rotation_reverse * intersections[l + 1]));
                        }
                    }
                }

                else {
                    for (T y = *y_minmax.first; y <= *y_minmax.second; y += _infill_dx) {
                        std::vector<Vector3<T>> intersections;
                        ...
                        for (int j = 0; j < contours_one_layer.size(); j++) {
                            ...
                            for (int k = 0; k < contour.size(); k++) {
                                ...
                                if ((point1[1] - y) * (point2[1] - y) <= 0) {
                                    Vector3<T> dir = point2 - point1;
                                    const T t = (y - point1[1]) / dir[1];
                                    intersections.push_back(point1 + dir * t);
                                }
                            }
                        }
